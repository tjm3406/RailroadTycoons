...
/**
 * Displays a {@linkplain PlayerPane pane} for each {@linkplain Player player} in the game. Each
 * pane includes the player's name, current score, and color and number of that player's remaining
 * train pieces.
 */
class ScoreBoard extends GridPane implements RailroadBaronsView {

  /**
   * Determines the number of columns into which the player panes should be divided.
   */
  private static final int MAX_COLUMNS = 2;

  /**
   * A {@link Map} of {@link Baron barons} to {@link PlayerPane player panes}. Used to update
   * individual panes with new information as the state of individual players changes.
   */
