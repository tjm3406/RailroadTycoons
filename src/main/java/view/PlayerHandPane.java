...
/**
 * Displays the current {@linkplain Player player's} hand as an {@linkplain CardView view} for each
 * type of {@linkplain model.Card card} with the number of cards in the hand indicated. Can flip
 * cards to be face down so that, as each player finishes their turn they do not see the cards in
...
public class PlayerHandPane extends GridPane implements RailroadBaronsView {
  ...
  /**
   * Creates a new {@link PlayerHandPane} with a {@link CardView} for each type of playable card.
   */
  PlayerHandPane() {
  ...
  /**
   * Updates the {@linkplain CardView card view} to display the appropriate number of each type of
   * card based on how many the specified {@linkplain Player player} has in hand.
   *
  ...
  /**
   * Flips all {@linkplain CardView card views} so that they are face down. Used when switching from
   * one {@linkplain Player player's} turn to another.
   */
  void placeFaceDown() {
  ...
  /**
   * Flips all of the {@linkplain CardView card views} so that they are face up. Used when a
   * {@linkplain Player player} starts a turn.
   */
  void placeFaceUp() {
    for (CardView view : cards) {
      view.placeFaceUp();
  ...
  /**
   * Helper method that adds a {@linkplain CardView card view} for the specified type of card.
   *
